...
let stubCalendarService = {
  calculateNextMonth: (): YearAndMonth => {
    return new YearAndMonth(NEXT_YEAR, NEXT_MONTH);
  },
  calculatePreviousMonth: (): YearAndMonth => {
    return new YearAndMonth(PREV_YEAR, PREV_MONTH);
  }
};
...
describe('Component: MonthHeader', () => {
  ...
  it('should populate the next month values from the CalendarService', () => {
    ...
    spyOn(calendarService, 'calculateNextMonth').and.callThrough();

    fixture.detectChanges();
    expect(calendarService.calculateNextMonth).toHaveBeenCalledTimes(1);
  ...
  it('should populate the previous month values from the CalendarService', () => {
    ...
    calendarService.year = 2015;
    calendarService.monthName = 'June';
    spyOn(calendarService, 'calculatePreviousMonth').and.callThrough();

    fixture.detectChanges();
    expect(calendarService.calculatePreviousMonth).toHaveBeenCalledTimes(1);
